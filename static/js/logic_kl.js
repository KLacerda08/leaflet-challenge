  // Create a map object - simple version for testing 
...
d3.json(queryUrl, function(record) {
  ...
  // create bins for the depths, to assing colors based on categories: 
  // I don't like this.  It only gave me 8 bins and put so many in bin 1 (0 to 100).  
  ...
  // Using my own values:  less than 0, 20, 40, 60, 80, 100, greater than 100 
  // console.log(record.features[0].geometry["coordinates"][2])
  for (var i = 0; i < record.features.length; i++) {
    var color = "";
  ...
  L.circle([(record.features[i].geometry["coordinates"][1], record.features[i].geometry["coordinates"][0])], {
    ...
    fillColor: color,
    weight: 0.25,
    // Adjust radius and add multiplier for map scale
    radius: record.features[0].properties["mag"] * 1500
    }).bindPopup("<h1>" + "something" + "</h1> <hr> <h3> Something else: something else + </h3>").addTo(myMap);
  }
});
  // Add circles to map
